...
@far @interrupt void EXTI3_IRQHandler(void) 
{
	...
	if (swres == 0) SB1 ++;
	if (SB1 >= 5) SB1 = 0;
	while (!((GPIOD->IDR & (1 << 3)))){//нажатие кнопки SB2 изменение скорости
	if (butcount < 10000){
			butcount ++;
					}
		else {
...
void DS1307init (void){//
          ...
          xxx = I2C->SR3;//Очистка бита ADDR чтением регистра SR3
					...
					delay (350);//delay (450);
          //I2C->DR = 0b01011001;//&= ~(1 << 7);//отправка данных
...
//==========================ВЫВОД ФРЕЙМА====================
	...
	for ( ip = 0; ip < speed; ++ip){//400  speed
				...
				*/
					GPIOD->ODR &= ~(1<<6);
					delay (500);//скорость бега строки1000
					GPIOD->ODR |= (1<<6);
