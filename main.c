...
while (CLK->SWCR & (1<<1) == 1){}// Ждем готовности переключения
  ...
  GPIOD->CR1 |= (1<<6);//
	...
	//========================================================
		//----Разрешаем запись в область памяти DATA--------------
	FLASH->DUKR = 0b10101110;//1-ый ключ MASS
	FLASH->DUKR = 0b01010110;//2-ой ключ MASS
	alar = *address_1;
	//--------------------------------------------------------
	/*GPIOD->ODR |= (1<<6);
	delay (20);
	...
	*/
	...
	if (iii > 36) {
		...
		while (!((GPIOD->IDR & (1 << 1)))){//нажатие кнопки
		...
		delay (5000);//delay (250);
					...
					//while(I2C->SR1 & (1 << 0) != 1){}//ждём установки стартового бита 1
					delay (18);//delay (50);
					I2C->DR = dev_addrr;//адрес часовой микросхемы - чтение
					while(I2C->SR1 & (1 << 1) == 1){}//ждём конца передачи адрес 1
					while(I2C->CR2 & (1 << 2) == 1){}//получили ACK 1
					delay (100);//delay (100);
					//disableInterrupts(); 
