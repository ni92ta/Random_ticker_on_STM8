...
while ((I2C->SR3 & (1 << 0))){}//ждём освобождения шины I2C
I2C->CR2 |= (1<<0);//отправка посылки СТАРТ
while (!((I2C->SR1 & (1 << 0)))){}//ждём установки стартового бита
I2C->DR = dev_addrw;//адрес часовой микросхемы - запись
/*while (!((I2C->SR1 & (1 << 1)))){}//ждём конца передачи адреса
I2C->SR3;//Очистка бита ADDR чтением регистра SR3
while (!((I2C->SR1 & (1 << 7)))){}//ждём освобождения регистра данных	
...
while (CLK->SWCR & (1<<1) == 1){}// Ждем готовности переключения
  ...
  GPIOD->CR1 |= (1<<6);//
	...
	//===================Настройка I2C========================
	I2C->FREQR = 0b00001100;
	I2C->CR1 &= ~(1<<0); 
	I2C->CCRH &= ~(1<<7);
	I2C->CCRH |= (1<<3) | (1<<1);
	I2C->TRISER |= (1<<3) | (1<<2) | (1<<0);
